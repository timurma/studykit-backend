...
description = <<-TEXT
  ...
  Проверка вручную означает, что ваш скрипт прочитает один из разработчиков Studykit и поставит отметку *верно* или *неверно*.
  Если хотите, чтобы задачу проверили ещё быстрее, напишите https://t.me/tpltn
  А задачу невозможно проверить потому что SQLite не позволяет удалять столбцы.

  Теперь сама задача.
  Удалим те столбцы, которые мы вынесли в таблицу пассажиров (имя пассажира, номер паспорта и контакты).
  В таблице билетов они называются **passenger_id**, **passenger_name** и **contact_data**.
TEXT
