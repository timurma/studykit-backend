...
description = <<-TEXT
  Теперь укажем ссылки в столбец **passenger_ref**.
  При этом, на всякий случай, для тех пассажиров, которых мы уже обновили, запишем "OK" вместо номера паспорта **passenger_id**.

  Примечание.
  Проверяющая система работает на SQLite (да-да, их синтаксис так сильно похож с PostgreSQL).
  Её не удалось заставить правильно обрабатывать обновления кортежами **SET (a, b) = (x, y)**, поэтому используйте **SET a = x, b = y**.
TEXT
