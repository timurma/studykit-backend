...
description = <<-TEXT
  Теперь сделаем так, чтобы в таблице **tickets** были ссылки на новую таблицу **passengers**.
  Для этого создадим столбец **passenger_ref** и объявим его внешней ссылкой для таблицы пассажиров.
  При удалении записей из таблицы **passengers** данные в **tickets** должны сохраняться с обнулением,
    при обновлении - должны обновляться.
TEXT
