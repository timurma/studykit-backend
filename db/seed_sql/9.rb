...
description = <<-TEXT
  Выбрать среднюю цену бронирования на билеты, по которым есть несколько рейсов
    (т.е. на один билет несколько рейсов. Например, билет туда-обратно, или билет, включающий пересадки).
  При этом, не учитывать самые дешёвые и самые дорогие бронирования (среди всех бронирований).

  Подсказка: бронирования могут включать несколько билетов,
    каждый билет из такого бронирования может быть на несколько рейсов.
  Бронирование, которое включает несколько таких билетов в результат должно попасть только один раз!
  ...
  Для рассчёта среднего *Бронирование1* берите только один раз.
TEXT
